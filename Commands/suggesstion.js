...
module.exports = {
  ...
  **Admin Commands**
  ...
  if(command.toLowerCase() === 'suggest') {
    ...
    if(!suggest) return message.channel.send(`${prefix}Suggestion suggest <Your Suggestion>`)
...
if(!checking) return message.channel.send(`i can't find suggestion room please set a new one...`)
if(checking) {
   ...
   .setTimestamp()
    checking.messages.fetch(messageid).then(editm => {
        editm.edit(embed)
        client.users.cache.get(user).send(`You've got answer on your suggestion by admin.`)
